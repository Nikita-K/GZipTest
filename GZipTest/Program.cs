...
Args description:
    ...
    {ex.Message}");
        ...
        static int RunGzipPipeline(Arguments parsedArgs)
        {
            ...
            {
                var gzipPipeline = new GZipPipeline(_parsedArgs.Mode, inputStream, outputStream);

                gzipPipeline.Start(_tokenSource.Token);
                gzipPipeline.Completion.Join();

                if (gzipPipeline.Exception != null)
                {
                    Console.WriteLine();
                    Console.WriteLine($@"Fail to {_parsedArgs.Mode} input file:
    {gzipPipeline.Exception.Message}");
                    return 1;
                }
